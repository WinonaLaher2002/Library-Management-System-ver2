...
from BB import create_borrow_book_window as create_admin_borrow_window
from RB import create_return_book_window as create_admin_return_window
...
class AdminView(tk.Toplevel):
    ...
    def _build_ui(self):
        ...
        delete_button = tk.Button(self, text="Delete Book", command=self.delete_book, fg="white", bg="#135D66")
        delete_button.grid(row=10, column=0, columnspan=3, padx=10, pady=5, sticky="ew")
        ...
        self.treeview.grid(row=11, column=0, columnspan=3, padx=10, pady=5, sticky="nsew")
    ...
    def borrow_book(self):
        create_admin_borrow_window(self)

    def return_book(self):
        create_admin_return_window(self)
    ...
    def delete_book(self):
        ...
        if not selected_item:
            messagebox.showerror("Error", "Please select a book to delete.", parent=self)
            return

        book_id = self.treeview.item(selected_item, "values")[0]
        try:
...
def create_admin_view(master=None, on_logout=None):
    return AdminView(master=master, on_logout=on_logout)


if __name__ == "__main__":
    root = tk.Tk()
    root.withdraw()
    window = create_admin_view(master=root, on_logout=root.destroy)
    window.wait_window()
